...
if __name__ == "__main__":
    for species in tqdm_:
        ...
        for image in list(species.glob("*/*")):
            ...
            mask_fil = cv2.bilateralFilter(cv2.cvtColor(mask, cv2.COLOR_GRAY2BGR), 20, 121, 34)
            new_path = dest_path / image.relative_to(image.parts[0])
            new_path.parent.mkdir(parents=True, exist_ok=True)
            cv2.imwrite(new_path.resolve(), mask_fil)